...
def random_weights(
    ...
    if spectral_radius is not None and input_size != output_size:
        raise ValueError(
            "spectral_radius can be defined only for square matrices (nbUnitsIN == nbUnitsOUT)"
        )
    ...
    if spectral_radius is not None:
        current_spec_rad = max(
            torch.abs(torch.linalg.eig(weights)[0])
        )
        weights = weights / current_spec_rad * spectral_radius
